...
describe("user services", () => {
    ...
    it("fetches all users for admin pages", async () => {
        ...
        vi.mocked(parseUsersResponse).mockReturnValueOnce([
            { id: "u1" },
        ] as never);
    ...
    it("fetches a public user by login", async () => {
        vi.mocked(api.get).mockResolvedValueOnce(publicUserPayload);

        await expect(fetchUserByLogin("Octo Cat")).resolves.toEqual(
            publicUserPayload,
        );
    ...
    it("falls back to post when the backend registered user lookup with a non-get method", async () => {
        ...
        vi.mocked(api.post).mockResolvedValueOnce(publicUserPayload);

        await expect(fetchUserByLogin("octocat")).resolves.toEqual(
            publicUserPayload,
        );
