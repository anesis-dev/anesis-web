...
export function OwnedStackCard({ stack }: { stack: IStack }) {
	...
	async function toggleVisibility() {
		...
		setError(null);
		try {
			await updateStackVisibility(stack.id, isPrivate ? "public" : "private");
			await refresh();
		} catch (err) {
	...
	return (
		<div className="flex h-full flex-col gap-3 rounded-xl border bg-card px-6 py-5">
			...
			<div className="mt-auto flex items-center gap-1 border-t pt-3">
				<Button asChild size="icon-xs" variant="ghost" title="View repository">
				...
				>
					{busy === "delete" ? <LoaderIcon className="animate-spin" /> : <Trash2Icon />}
				</Button>
			</div>
