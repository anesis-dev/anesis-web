		...
		<div className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-10 sm:px-5 lg:px-8">
			<div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
				<div className="max-w-3xl space-y-2">
					...
					<p className="text-sm leading-6 text-muted-foreground sm:text-base">
						The addon registry is now live in the web app. `oxide-server`
						already exposes publish, list, delete, and archive-url endpoints;
						this page surfaces discovery and publishing while the public CLI
						addon commands are still catching up.
					</p>
				</div>
			...
			{isError && (
				<div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-destructive/30 bg-destructive/5 py-16 text-center">
					...
					<div>
						<p className="font-medium text-sm">Failed to load addons</p>
			...
			{!isLoading && !isError && filtered.length === 0 && (
				<div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed py-16 text-center">
					...
					<div>
						<p className="font-medium text-sm">No addons found</p>
						<p className="mt-1 text-xs text-muted-foreground">
							{addons.length === 0
