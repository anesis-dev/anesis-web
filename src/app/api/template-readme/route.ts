...
async function loadReadmeContent(readme: GitHubContentEntry) {
    ...
    if (readme.url) {
        ...
        if (readmeApiResponse.ok) {
            const payload =
                (await readmeApiResponse.json()) as GitHubFileContent;
...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        const payload = (await contentsResponse.json()) as
            GitHubContentEntry[] | GitHubContentEntry;
        ...
        const message =
            error instanceof Error
                ? error.message
                : "Failed to load template README.";
